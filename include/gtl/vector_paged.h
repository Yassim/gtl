#ifndef GTL_VECTOR_PAGED
#define GTL_VECTOR_PAGED
    ...
    : private cfg_type::heap_base
{
    ...
    struct page {
        uint8_t space[kBlobSize];
        bit_set<kPageCount> used;
        page*               next;
    };
...
public:
    ...
    // Construction
    explicit base_vector_paged(const Allocator& i_heap)
        : base0_type(i_heap)
        , m_p(nullptr)
        , m_count(0)
        , m_capacity(0)
    {}
    base_vector_paged() : base_vector_paged(Allocator()) {}
//    explicit base_vector_paged(size_type count, const T& value = T(), const Allocator& alloc = Allocator());
    base_vector_paged(size_type count, const T& value, const Allocator& alloc = Allocator()) : base_vector_paged(alloc) { resize(count, value); }
    explicit base_vector_paged(size_type count) : base_vector_paged(Allocator()) { resize(count); }
    explicit base_vector_paged(size_type count, const Allocator& alloc = Allocator()) : base_vector_paged(alloc) { resize(count); }
    template< class InputIt >
    base_vector_paged(InputIt first, InputIt last, const Allocator& alloc = Allocator()) : base_vector_paged(alloc) { assign(first, last); }
    base_vector_paged(const base_vector_paged& other) : base_vector_paged(static_cast<const Allocator&>(other)) { assign(other.begin(), other.end()); }
    base_vector_paged(const base_vector_paged& other, const Allocator& alloc) : base_vector_paged(alloc) { assign(other.begin(), other.end()); }
//        base_vector_paged(base_vector_paged&& other)
//        base_vector_paged(base_vector_paged&& other, const Allocator& alloc);
//    base_vector_paged(std::initializer_list<T> init, const Allocator& alloc = Allocator());
    ...
    base_vector_paged& operator=(const base_vector_paged& other)
    {
        if (this != &other) {
            base_vector_paged(other).swap(*this);
        }
        return *this;
    }
    //base_vector_paged& operator=(base_vector_paged&& other);
    //base_vector_paged& operator=(std::initializer_list<T> ilist);
    ...
    void swap(base_vector_paged& i_rhs)
    {
...
class vector_paged : public base_vector_paged < vector_cfg< T, THeap > >
{
    typedef base_vector_paged < vector_cfg< T, THeap > > base;
public:
    typedef typename base::Allocator Allocator;
//    typedef typename base::ptr_lifetime ptr_lifetime;

    explicit vector_paged(const Allocator& i_heap) : base(i_heap)  {}
    vector_paged() : base(Allocator()) {}
    //    explicit base_vector_paged(size_type count, const T& value = T(), const Allocator& alloc = Allocator());
    vector_paged(size_type count, const T& value, const Allocator& alloc = Allocator()) : base(count, value, alloc) { }
    explicit vector_paged(size_type count) : base(count) {  }
    explicit vector_paged(size_type count, const Allocator& alloc = Allocator()) : base(count, alloc) { }
    template< class InputIt >
    vector_paged(InputIt first, InputIt last, const Allocator& alloc = Allocator()) : base(first, last, alloc) {  }
    vector_paged(const vector_paged& other) : base(other) {  }
    vector_paged(const vector_paged& other, const Allocator& alloc) : base(other, alloc) {  }
    //        vector_paged(vector_paged&& other)
    //        vector_paged(vector_paged&& other, const Allocator& alloc);
    //    vector_paged(std::initializer_list<T> init, const Allocator& alloc = Allocator());
    inline ~vector_paged() { }
};


template<typename T, class THeap> inline
const typename vector_paged<T, THeap>::iterator begin(vector_paged<T, THeap>& i_vector)
{
    return i_vector.begin();
}

template<typename T, class THeap> inline
const typename vector_paged<T, THeap>::iterator end(vector_paged<T, THeap>& i_vector)
{
    return i_vector.end();
}
...
namespace meta {
    ...
    struct lifetime_util_select< ::gtl::vector_paged<T, THeap> >
    {
        typedef typename ::gtl::vector_paged<T, THeap>::ptr_type          ptr_type;
        typedef typename lifetime_util_select< ptr_type >::type     type;
    };*/

    template<typename cfg_type>
    struct lifetime_util_select< ::gtl::base_vector_paged<cfg_type> >
    {
        typedef typename ::gtl::base_vector_paged<cfg_type>::ptr_type     ptr_type;
        typedef typename lifetime_util_select< ptr_type >::type     type;
    };
}
